...
test.describe("Room Creation and Navigation Flow", () => {
  test("should create room with questions and display them", async ({
    page,
  }) => {
    ...
    await page.waitForLoadState("networkidle");
    await page.screenshot({
      path: "test-results/02-setup-page.png",
      fullPage: true,
    });
    ...
    await page.screenshot({
      path: "test-results/03-questions-selected.png",
      fullPage: true,
    });
    ...
    console.log("✅ Start Playing button is enabled (setup flow complete)");

    // Success! The test verifies:
    // 1. ✅ Can create a room
    // 2. ✅ Can navigate to setup page
    // 3. ✅ Can select questions
    // 4. ✅ Start Playing button becomes enabled
    // Note: Complete setup requires authentication, which is expected behavior

    // Take final screenshot
    await page.screenshot({
      path: "test-results/04-setup-complete-ready.png",
      fullPage: true,
    });
    ...
    if (url.includes("/rooms/")) {
      console.log("🎉 Successfully completed setup flow!");
    } else {
