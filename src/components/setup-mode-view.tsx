...
export function SetupModeView({ roomId, onComplete }: SetupModeViewProps) {
  ...
  const [isCustomQuestionModalOpen, setIsCustomQuestionModalOpen] =
    useState(false);
  const [editingQuestion, setEditingQuestion] = useState<QuestionPrompt | null>(
    null,
  );
    ...
    const loadData = async () => {
      try {
        ...
        if (roomRes.ok) {
          ...
          console.error("Failed to load room data:", roomRes.statusText);
        }
  ...
  const handleUpdateQuestion = (updatedQuestion: QuestionPrompt) => {
    ...
    if (!customQuestions.some((q) => q.id === updatedQuestion.id)) {
      ...
      setSelectedQuestionIds((prev) =>
        prev.filter((id) => id !== updatedQuestion.id),
      );
      ...
      const response = await fetch(`/api/rooms/${roomId}/complete-setup`, {
        ...
        body: JSON.stringify({
          ...
          customQuestions: customQuestions.map((q) => ({
            ...
            slider:
              q.answerConfig?.type === "slider"
                ? q.answerConfig.config
                : undefined,
            multipleChoice:
              q.answerConfig?.type === "multipleChoice"
                ? q.answerConfig.config
                : undefined,
  ...
  const unselectedQuestions = allQuestions.filter(
    ...
    <div className="min-h-screen bg-background art-deco-pattern p-6">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="text-center mb-12">
          <h1 className="text-4xl font-serif mb-3 art-deco-text">
            Setup Your Room
          </h1>
          <div className="flex items-center justify-center gap-2 text-primary/50 mb-6">
            <span>◆</span>
            <span>◆</span>
            <span>◆</span>
          </div>
        ...
        <div className="max-w-xl mx-auto mb-12">
          <div className="bg-card/50 backdrop-blur-sm p-6 rounded-xl border border-border">
            ...
            <div className="flex gap-3">
              <input
                ...
                className="flex-1 bg-secondary border border-input rounded-md px-4 py-2 text-foreground focus:outline-none focus:ring-2 focus:ring-primary/50"
                placeholder="Enter a name for your room"
              />
              <Button
                onClick={handleRename}
                disabled={isRenaming || !roomName.trim()}
            ...
            {/* Invite Link Preview */}
            <div className="mt-4 p-3 bg-secondary/30 rounded-lg border border-primary/20">
              <p className="text-xs text-primary mb-1">Invite Link Preview</p>
              <p className="font-mono text-sm text-muted-foreground break-all">
                {typeof window !== "undefined" ? window.location.origin : ""}
                /invite/
                <span className="text-foreground font-bold">
                  {roomName.trim().replace(/\s+/g, "-") || "..."}
                </span>
              </p>
            </div>
          </div>
          ...
          <p className="text-lg text-muted-foreground text-center mb-4">
            Select questions for your group to answer. You can pick suggested
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div>
          <h2 className="text-2xl font-serif mb-4 art-deco-text">
            Suggested Questions
          </h2>
          <div className="space-y-3 max-h-[600px] overflow-y-auto pr-2">
            ...
            {hasMoreQuestions && (
              ...
              >
                Load more questions (
                {unselectedQuestions.length - visibleQuestionCount} remaining)
              </button>
            )}
          </div>
        </div>
      ...
      <CustomQuestionModal
        ...
        onCreateQuestion={
          editingQuestion ? handleUpdateQuestion : handleCreateCustomQuestion
        }
