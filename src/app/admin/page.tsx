...
export default function AdminPage() {
  ...
  useEffect(() => {
    ...
    filtered = showArchived
      ? getArchivedQuestions(filtered)
      : getActiveQuestions(filtered);

    // Filter by selected tags
    if (selectedTags.length > 0) {
      filtered = filtered.filter(
        (q) => q.tags && q.tags.some((tag) => selectedTags.includes(tag.name)),
      );
    }
  ...
  const handleCreateQuestion = () => {
    const tagsArray = newQuestion.tags
      ...
      .map((tag) => ({ name: tag, type: "topic" as const }));
  ...
  const handleUpdateSpiciness = (questionId: string, spiciness: number) => {
    setQuestions((prev) =>
      updateQuestion(questionId, prev, { suggestedLevel: spiciness }),
    );
  };
  ...
  const handleUpdateTags = (questionId: string, tags: string[]) => {
    ...
    setQuestions((prev) =>
      updateQuestion(questionId, prev, { tags: tagObjects }),
    );
  };
    ...
    <div className="min-h-screen bg-background art-deco-pattern p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-4xl font-serif text-foreground art-deco-text art-deco-shadow">
              Question Admin
            </h1>
            <p className="text-muted-foreground mt-2">
              Manage question cards for your Secret Game
            </p>
          ...
          <Button
            ...
            className="flex items-center gap-2 art-deco-border bg-primary text-primary-foreground hover:bg-primary/90 art-deco-text art-deco-glow"
          >
            ...
            {allTags.length > 0 && (
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  {allTags.map((tag) => {
                    const isCategoryTag = QUESTION_CATEGORIES.some(
                      (cat) => cat.toLowerCase() === tag,
                    );
                    return (
                      <Badge
                        key={tag}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {currentQuestions.map((question) => (
            <AdminQuestionCard
              ...
              onUpdateSpiciness={(spiciness) =>
                handleUpdateSpiciness(question.id, spiciness)
              }
              ...
              onToggleRoomSelection={() =>
                handleToggleQuestionInRoom(question.id)
              }
...
function AdminQuestionCard({
  ...
  const [tagInput, setTagInput] = useState(
    question.tags
      ?.filter((tag) => tag.name !== question.category.toLowerCase())
      .map((tag) => tag.name)
      .join(", ") || "",
  );

  const handleSaveTags = () => {
    const additionalTags = tagInput
      .split(",")
      .map((tag) => tag.trim())
      .filter((tag) => tag.length > 0);
  ...
  return (
    <Card
      className={`p-4 ${getCategoryColor(question.category)} ${isArchived ? "opacity-60" : ""} ${isSelectedForRoom ? "ring-2 ring-blue-500" : ""}`}
    >
          ...
          <div className="flex items-center gap-2">
            {showRoomSelection && (
              <button
                ...
                style={{
                  backgroundColor: isSelectedForRoom
                    ? "#3B82F6"
                    : "transparent",
                  borderColor: isSelectedForRoom ? "#3B82F6" : undefined,
                }}
              >
                {isSelectedForRoom && <Check className="w-3 h-3 text-white" />}
              </button>
            )}
                ...
                <Button
                  ...
                  onClick={() => {
                    ...
                    setTagInput(
                      question.tags
                        ?.filter(
                          (tag) => tag.name !== question.category.toLowerCase(),
                        )
                        .map((tag) => tag.name)
                        .join(", ") || "",
                    );
            ...
            <div className="flex flex-wrap gap-1">
              {question.tags && question.tags.length > 0 ? (
                question.tags.map((tag, index) => {
                  const isCategoryTag =
                    tag.name === question.category.toLowerCase();
                  return (
                    <Badge
                      key={index}
